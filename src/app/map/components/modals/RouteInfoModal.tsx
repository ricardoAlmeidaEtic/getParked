...
export default function RouteInfoModal({
  ...
  useEffect(() => {
    if (isReservationModalOpen) {
      setShouldRenderReservationModal(true)
    } else {
      const timer = setTimeout(() => {
        setShouldRenderReservationModal(false)
      }, 300)
      return () => clearTimeout(timer)
    }
  ...
  if (!shouldRender && !shouldRenderReservationModal) return null
